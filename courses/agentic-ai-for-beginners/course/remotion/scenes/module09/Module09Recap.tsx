...
interface Module09RecapProps extends SceneProps {}

export const Module09Recap: React.FC<Module09RecapProps> = ({ durationInFrames }) => {
	...
	const examTerms = [
		...
		'Triton dynamic batching and concurrency',
		'when TensorRT applies',
	];
	const recapPoints = [
		'GPUs favor batched parallel inference.',
		'Triton batching and concurrency raise throughput; TensorRT cuts latency.',
		'Measure baseline, then optimize for your agent workload.',
	];
	const points = isExamSegment ? examTerms : recapPoints;
	const title = isExamSegment ? 'Exam Focus' : 'Key Takeaways';
	const subtitle = isExamSegment ? 'Certification terms and scenario matching' : 'Module 09: NVIDIA Inference Optimization';
