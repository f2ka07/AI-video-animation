...
interface Module10RecapProps extends SceneProps {}

export const Module10Recap: React.FC<Module10RecapProps> = ({ durationInFrames }) => {
	...
	const examTerms = [
		'ingestion through grounding pipeline',
		'retrieval vs tool calling',
		'schema enforcement and hallucination reduction',
	];
	const recapPoints = [
		'Pipeline: ingest, chunk, embed, search, inject.',
		'Retrieval for knowledge; tools for actions; use both when needed.',
		'Cite sources and enforce schemas on every tool call.',
	];
	const points = isExamSegment ? examTerms : recapPoints;
	const title = isExamSegment ? 'Exam Focus' : 'Key Takeaways';
	const subtitle = isExamSegment ? 'Certification terms and scenario matching' : 'Module 10: Knowledge Integration and RAG';
