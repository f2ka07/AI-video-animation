// Extract word timings from existing audio files using OpenAI Whisper API
...
/**
 * Main function to extract word timings for all slides using OpenAI Whisper API
 */
async function extractAllWordTimings() {
	...
	if (!openaiKey) {
		...
		console.error("\nThen run:");
		console.error("  npm run extract-timings\n");
		process.exit(1);
	}
