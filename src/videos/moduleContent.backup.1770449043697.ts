...
// Activated: 2026-02-07T05:00:50.861Z
...
export const module1Content: ModuleContent = {
	...
	slides: [
		...
		},
		{
			...
			type: "code",
			script: "This Python script scans for available Wi-Fi networks using the platform's wireless tools. First, it runs a system command to list networks. Then, it parses and prints the network names, helping you see nearby wireless options. This code runs a command-line tool to list Wi-Fi networks. It captures the output, decodes it from bytes to text, and separates each network name. Finally, it prints each network. This shows how software can interact with wireless hardware to discover networks.",
			title: "Scan Wi-Fi Networks with Python",
			code: "# Runs command-line Wi-Fi scan\nimport subprocess\n\n# Run system command to scan Wi-Fi networks\nresult = subprocess.run(['nmcli', '-t', '-f', 'SSID', 'device', 'wifi', 'list'], stdout=subprocess.PIPE)\n\n# Decode and split output\nnetworks = result.stdout.decode().split('\\n')\n\n# Print available networks\nfor network in networks:\n    if network:\n        print(f'Found network: {network}')",
			language: "python",
		},
		{
...
export const module2Content: ModuleContent = {
	...
	slides: [
		...
		},
		{
			...
			type: "code",
			script: "This Python example uses the 'subprocess' module to configure a wireless access point on Linux. It sets the SSID and password via 'nmcli' commands, demonstrating how scripts can automate network setup tasks. This script sets up a Wi-Fi hotspot by running a system command. It specifies the network interface, SSID, and password. Automating these steps makes configuring networks faster and reduces errors compared to manual setup.",
			title: "Configure Wireless Access Point with Python",
			code: "# Runs system command to set hotspot\nimport subprocess\n\n# Specifies network interface wlan0\nssid = 'MyNetwork'\npassword = 'SecurePass123'\n\n# Set SSID\nsubprocess.run(['nmcli', 'device', 'wifi', 'hotspot', 'ifname', 'wlan0', 'ssid', ssid, 'password', password])\nprint(f'Configured AP with SSID: {ssid}')",
			language: "python",
		},
		{
		...
		},
		{
			...
			type: "code",
			script: "This Python code scans available Wi-Fi channels and shows their signal strengths. It helps identify which channels are crowded so you can pick a better one for your network. The code runs a wireless scan command and decodes its output. It searches for lines mentioning 'Channel:' to collect channel info. Printing this lets you see which frequencies wireless devices nearby use, guiding your channel choice.",
			title: "Scan Wi-Fi Channels with Python",
			code: "# Runs wireless scan command\nimport subprocess\n\n# Decodes command output\nresult = subprocess.run(['iwlist', 'wlan0', 'scan'], stdout=subprocess.PIPE)\noutput = result.stdout.decode()\n\n# Finds lines with 'Channel:'\nchannels = []\nfor line in output.split('\\n'):\n    if 'Channel:' in line:\n        channels.append(line.strip())\n\n# Lists detected channels\nprint('Detected Wi-Fi Channels:')\nfor ch in channels:\n    print(ch)",
			language: "python",
		},
	]
};
...
export const module3Content: ModuleContent = {
	...
	slides: [
		...
		},
		{
			...
			type: "code",
			script: "This Python example demonstrates sending and receiving data over a wireless network using UDP sockets. One script sends a message, and another listens and prints it, showing basic wireless data transmission in action. This code creates a UDP socket to send a broadcast message. It defines the broadcast IP and port, then sends a simple text message. UDP is connectionless, making it fast for real-time wireless communication but without guaranteed delivery.",
			title: "Send and Receive Data with UDP in Python",
			code: "# Creates UDP socket\nimport socket\n\n# Sender\nsender = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)\nmessage = b'Hello Wireless World'\ndest = ('192.168.1.255', 5005)  # Broadcast address\nsender.sendto(message, dest)\nsender.close()",
			language: "python",
		},
		{
			name: "receive-udp-python",
			type: "code",
			script: "Here is the listening Python script for UDP. It binds to a port and waits for messages. When a message arrives, it prints the sender’s address and the content, illustrating simple wireless reception. This script listens on a UDP port for incoming messages. It waits indefinitely and prints information when data arrives. This models how wireless devices can receive broadcast messages without needing a fixed connection.",
			title: "Receive UDP Messages in Python",
			code: "# Binds to UDP port\nimport socket\n\n# Waits for incoming messages\nreceiver = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)\nreceiver.bind(('0.0.0.0', 5005))\n\n# Prints sender and content\nprint('Listening for UDP messages...')\nwhile True:\n    data, addr = receiver.recvfrom(1024)\n    print(f'Received from {addr}: {data.decode()}')",
			language: "python",
		},
		{
...
export const module4Content: ModuleContent = {
	...
	slides: [
		...
		},
		{
			...
			type: "code",
			script: "This Python example simulates a simple mesh node communicating with neighbors. It sends messages to nodes in its list and listens for incoming ones, illustrating basic mesh communication principles. This script starts a UDP socket to listen for messages and sends a greeting to neighbor nodes. Using threads allows simultaneous listening and sending. This models how mesh nodes exchange data dynamically to maintain the network.",
			title: "Simulate Mesh Node Communication in Python",
			code: "# Starts UDP socket for listening\nimport socket\nimport threading\n\n# Sends message to neighbors\nneighbors = [('192.168.1.2', 5005), ('192.168.1.3', 5005)]\n\n# Uses threading for concurrency\nsock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)\nsock.bind(('0.0.0.0', 5005))\n\n# Models dynamic mesh communication\ndef listen():\n    while True:\n        data, addr = sock.recvfrom(1024)\n        print(f'Message from {addr}: {data.decode()}')\n\ndef send_to_neighbors(msg):\n    for n in neighbors:\n        sock.sendto(msg.encode(), n)\n\nthreading.Thread(target=listen, daemon=True).start()\nsend_to_neighbors('Hello from mesh node')",
			language: "python",
		},
		{
		...
		},
		{
			...
			type: "code",
			script: "This example shows how to send a message over a Zigbee network using a Python library. It initializes the Zigbee device and sends data to a target node, demonstrating wireless IoT communication basics. This code opens communication with a Zigbee device on a serial port. It discovers a remote device by address and sends a text message if found. This example highlights how Python can control wireless IoT devices.",
			title: "Send Zigbee Message with Python",
			code: "# Opens Zigbee device connection\nfrom digi.xbee.devices import XBeeDevice\n\n# Discovers remote device\nPORT = '/dev/ttyUSB0'\nBAUD_RATE = 9600\n\n# Sends message if device found\ndevice = XBeeDevice(PORT, BAUD_RATE)\ndevice.open()\n\n# Controls wireless IoT devices\nremote_device = device.get_network().discover_device('0013A20040B41234')\nif remote_device:\n    device.send_data(remote_device, 'Hello Zigbee')\n    print('Message sent')\ndevice.close()",
			language: "python",
		},
		{
