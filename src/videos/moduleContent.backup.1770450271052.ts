...
// Activated: 2026-02-07T07:44:28.674Z
...
export const module1Content: ModuleContent = {
	...
	slides: [
		...
		},
		{
			...
			type: "code",
			scripts: ["This Python script scans for Wi-Fi networks on Windows, Linux, and macOS. It detects the OS and runs the appropriate command.","On Linux we use nmcli. On Windows we use netsh. On macOS we use the airport command.","The output is decoded, parsed, and each network name is printed."],
			title: "Scan Wi-Fi Networks with Python",
			code: "# Detect OS and run appropriate Wi-Fi scan command\nimport subprocess\nimport sys\n\ndef scan_wifi():\n    if sys.platform == 'win32':\n        # Windows: netsh wlan show networks\n        result = subprocess.run(\n            ['netsh', 'wlan', 'show', 'networks'],\n            capture_output=True, text=True\n        )\n        lines = result.stdout.split('\\n')\n        networks = []\n        for line in lines:\n            if 'SSID' in line and ':' in line and 'BSSID' not in line:\n                parts = line.split(':', 1)\n                if len(parts) == 2:\n                    networks.append(parts[1].strip())\n    elif sys.platform == 'darwin':\n        # macOS: airport -s\n        path = '/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport'\n        result = subprocess.run(\n            [path, '-s'],\n            capture_output=True, text=True\n        )\n        out = result.stdout.strip().split('\\n')\n        networks = [line.split()[0] for line in out[1:] if line.strip()]\n    else:\n        # Linux: nmcli\n        result = subprocess.run(\n            ['nmcli', '-t', '-f', 'SSID', 'device', 'wifi', 'list'],\n            capture_output=True, text=True\n        )\n        networks = result.stdout.strip().split('\\n')\n\n    for network in networks:\n        if network:\n            print(f'Found network: {network}')\n\nscan_wifi()",
			language: "python",
		},
		{
...
export const module2Content: ModuleContent = {
	...
	slides: [
		...
		},
		{
			...
			type: "code",
			scripts: ["This Python example configures a Wi-Fi hotspot on Windows, Linux, and macOS. It detects the OS and runs the appropriate command.","On Linux we use nmcli. On Windows we use netsh. On macOS we use networksetup or system commands.","The SSID and password are set, and the hotspot is started."],
			title: "Configure Wireless Access Point with Python",
			code: "# Detect OS and run appropriate hotspot command\nimport subprocess\nimport sys\n\nssid = 'MyNetwork'\npassword = 'SecurePass123'\n\nif sys.platform == 'win32':\n    # Windows: netsh wlan set hostednetwork\n    subprocess.run(\n        ['netsh', 'wlan', 'set', 'hostednetwork', 'mode=allow', f'ssid={ssid}', f'key={password}'],\n        check=True\n    )\n    subprocess.run(['netsh', 'wlan', 'start', 'hostednetwork'], check=True)\nelif sys.platform == 'darwin':\n    # macOS: networksetup (requires admin)\n    subprocess.run(\n        ['networksetup', '-setairportnetwork', 'en0', ssid, password],\n        capture_output=True\n    )\nelse:\n    # Linux: nmcli\n    subprocess.run(\n        ['nmcli', 'device', 'wifi', 'hotspot', 'ifname', 'wlan0', 'ssid', ssid, 'password', password],\n        check=True\n    )\n\nprint(f'Configured AP with SSID: {ssid}')",
			language: "python",
		},
		{
		...
		},
		{
			...
			type: "code",
			scripts: ["This Python code scans Wi-Fi channels on Windows, Linux, and macOS. It detects the OS and runs the appropriate scan command.","On Linux we use iwlist. On Windows we use netsh. On macOS we use airport -s.","The output is decoded, parsed for channel info, and printed."],
			title: "Scan Wi-Fi Channels with Python",
			code: "# Detect OS and run appropriate channel scan command\nimport subprocess\nimport sys\n\nif sys.platform == 'win32':\n    result = subprocess.run(\n        ['netsh', 'wlan', 'show', 'networks', 'mode=bssid'],\n        capture_output=True, text=True\n    )\n    output = result.stdout\nelif sys.platform == 'darwin':\n    path = '/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport'\n    result = subprocess.run([path, '-s'], capture_output=True, text=True)\n    output = result.stdout\nelse:\n    result = subprocess.run(\n        ['iwlist', 'wlan0', 'scan'],\n        capture_output=True, text=True\n    )\n    output = result.stdout\n\nchannels = []\nfor line in output.split('\\n'):\n    if 'Channel:' in line:\n        channels.append(line.strip())\n\nprint('Detected Wi-Fi Channels:')\nfor ch in channels:\n    print(ch)",
			language: "python",
		},
	]
};
...
export const module3Content: ModuleContent = {
	...
	slides: [
		...
		},
		{
			...
			type: "code",
			scripts: ["This Python example sends data over a wireless network using UDP sockets. UDP works on Windows, Linux, and macOS.","We create a UDP socket and send a broadcast message to the network.","UDP is connectionless, making it fast for real-time wireless communication."],
			title: "Send and Receive Data with UDP in Python",
			code: "# Creates UDP socket\nimport socket\n\n# Sender: broadcast message to network\nsender = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)\nmessage = b'Hello Wireless World'\ndest = ('192.168.1.255', 5005)\nsender.sendto(message, dest)\nsender.close()",
			language: "python",
		},
		{
			name: "receive-udp-python",
			type: "code",
			scripts: ["This Python script listens for UDP messages. It binds to a port and waits for incoming data.","When a message arrives, it prints the sender address and content.","This models how wireless devices receive broadcast messages."],
...
export const module4Content: ModuleContent = {
	...
	slides: [
		...
		},
		{
			...
			type: "code",
			scripts: ["This Python example simulates a mesh node communicating with neighbors. It uses UDP sockets, which work on all platforms.","A UDP socket listens for messages while a thread sends greetings to neighbor nodes.","This models how mesh nodes exchange data dynamically."],
		...
		},
		{
			...
			type: "code",
			scripts: ["This example sends a Zigbee message using the digi-xbee library. Serial port paths differ by OS: COM on Windows, ttyUSB on Linux, tty.usbserial on macOS.","We open the device, discover the remote node, and send a message.","This demonstrates how Python controls wireless IoT devices."],
			title: "Send Zigbee Message with Python",
			code: "# Detect serial port path by OS\nimport sys\nfrom digi.xbee.devices import XBeeDevice\n\nif sys.platform == 'win32':\n    PORT = 'COM3'\nelif sys.platform == 'darwin':\n    PORT = '/dev/tty.usbserial-0001'\nelse:\n    PORT = '/dev/ttyUSB0'\n\nBAUD_RATE = 9600\ndevice = XBeeDevice(PORT, BAUD_RATE)\ndevice.open()\n\nremote_device = device.get_network().discover_device('0013A20040B41234')\nif remote_device:\n    device.send_data(remote_device, 'Hello Zigbee')\n    print('Message sent')\ndevice.close()",
			language: "python",
		},
		{
